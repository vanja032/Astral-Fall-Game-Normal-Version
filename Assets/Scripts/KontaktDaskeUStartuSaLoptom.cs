...
public class KontaktDaskeUStartuSaLoptom : MonoBehaviour {
    public float xd;
    public float yd;
    public float zd;
    public bool pr;
    ...
    private void OnCollisionStay(Collision collision1)
    {
        if (collision1.gameObject.tag == "EditorOnly" || collision1.gameObject.tag == "Finish")
        {
            pr = true;
            Instantiate(boxes, new Vector3(xd, yd, zd), Quaternion.identity);
            Instantiate(boxes, new Vector3(xd, yd, zd), Quaternion.identity);
