...
public class AddingPrepreke : MonoBehaviour {
    ...
    public float zposforgoldballs = 15;
	...
	void Update () {
        //pmfloat= PlayerPrefs.GetFloat("pizidibizi", pmfloat);
        w78++;
        if(ball.transform.position.z >= zpos - 200)
        {
        ...
        if (ball.transform.position.z>=zposforball-350)
        {
        ...
        if (ball.transform.position.z >= zposforgoldballs - 300)
        {
